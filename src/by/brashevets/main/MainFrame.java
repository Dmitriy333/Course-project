		...
		btnEndDeal.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					if(row > 0 && column >0){
						...
						JOptionPane.showMessageDialog(null, "Выберите дело, чтобы пометить, как 'завершенное'","Выберите дело", 1);
					}
			}
		});
		...
		btnEditDeal.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(row >= 0 && table.getValueAt(row, 0)!=null){
					...
					JOptionPane.showMessageDialog(null, "Выберите дело ", "Некорректное действие", 1);
				}
			}
		});
